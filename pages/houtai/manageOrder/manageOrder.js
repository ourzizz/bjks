...
Page({
    ...
    connect  () {
        ...
        tunnel.on('picked', picked => {
            var index = findOrderById(this.data.wait_pick_order_list,picked.order_id)
            if(index !== -1){
        ...
        tunnel.on('refund', refund => {
            var refund_order_id = refund.order_id
            var index = 0
        ...
        tunnel.on('user_signed', user_signed => {
            ...
            var order_id = user_signed.order_id
            var index = 0
            ...
            var delivery_idx = findOrderById(this.data.wait_delivery_order_list,order_id);
        ...
        tunnel.on('cancle_delivery', cancle_delivery => {
            //bug定位到本段应该是index没有找对
            var order_id = cancle_delivery.order_id
            var index = 0
            if((index = findOrderById(this.data.wait_refund_list,order_id)) !== -1){
        ...
        tunnel.on('delivery', delivery => {
            ...
            var order_id = delivery.order_info.order.order_id
            var index = 0
